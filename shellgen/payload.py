...
def shell(arch, os, **options):
    ...
    Typically used for **network service** exploits.
    ...
    :keyword connect: Communication channel.
        ...
        - *bind_tcp*: Listen on the given TCP port for incoming connections.
        - *reverse_tcp*: Connect back to the given TCP port on this machine.
        - *reuse_tcp*: Reuse an existing TCP socket (for network services).
        - *none*: Do not use a communication channel (for inetd programs). Defaults to *bind_tcp*.

    :type    address: str
    :keyword address: IP address to bind to (*bind_tcp*) or connect to
        (*reverse_tcp*, *reuse_tcp*). For *bind_tcp* the default is
        *"0.0.0.0"*. Unused for *none*.

    :type    port: int
    :keyword port: TCP port for the communication channel (*bind_tcp*,
        *reverse_tcp*). Unused for *reuse_tcp and *none*.

    :type    staged: bool
    :keyword staged: *True* to split execution into two stages, *False*
        ...
        not support this option. Defaults to *False*.
...
def download_exec(arch, os, url, **options):
    ...
    Typically used for **browser** or **file** exploits.
...
def execute(arch, os, command, **options):
    ...
    Typically used for **privilege escalation** exploits.
...
def adduser(arch, os, **options):
    ...
    :type    username: str
    :keyword username: Username to add. Defaults to *"newuser"*.

    :type    password: str
    :keyword password: Password to the new user. Defaults to *"1234"*.
...
def chmod(arch, os, **options):
    ...
    :type    mode: int
    :keyword mode: Octal mode for the file. Defaults to *6777*.
...
def xor_encode(payload, bad_chars = "\0\r\n\x1a\"'`%,;:."):
    ...
    :type    bad_chars: str
    :keyword bad_chars: String containing all the characters that must be avoided.
        Defaults to the following characters: *00 0A 0D 1A 22 25 27 2C 2E 3A 3B 60*
...
def ascii_encode(payload, allow_upper = True,
    ...
    :param allow_upper: *True* if uppercase letters are allowed, *False*
        otherwise. Defaults to *True*.

    :type  allow_lower: bool
    :param allow_lower: *True* if uppercase letters are allowed, *False*
        otherwise. Defaults to *True*.
...
def unicode_encode(payload, allow_upper = True,
    ...
    :param allow_null: *True* if null wchars are allowed, *False* otherwise.
        Defaults to *False*.
